defmodule ExMicrosoftBot.Client do
  ...
  * `ExMicrosoftBot.Client.Teams`
  ...
  def put(url, body, extra_opts) when is_map(body) do
    ...
    put(url, Poison.encode!(body), extra_opts)
  end
  ...
  defp denullify_request_body(%_{} = struct),
    do: struct |> Map.from_struct() |> denullify_request_body()

  defp denullify_request_body(%{} = map),
    do:
      :maps.filter(fn _, v -> v != nil end, map)
      |> Enum.map(&denullify_request_body/1)
      |> Enum.into(%{})

  defp denullify_request_body(list) when is_list(list),
    do: Enum.map(list, &denullify_request_body/1)

  defp denullify_request_body(tuple) when is_tuple(tuple),
    do:
      tuple
      |> Tuple.to_list()
  ...
  defp denullify_request_body(value), do: value

  defp http_client_opts, do: Application.get_env(:ex_microsoftbot, :http_client_opts, [])
end
